 ...
 */
 ...
 (function (exports) {
  "use strict";
    ...
    // the model for the JW playlist data
    ...
    function JSONMediaModel(appSettings) {
         ...
         */
         this.loadInitialData = function (dataLoadedCallback) {
          ...
          for(var i = 0; i < appSettings.dataURL.length; i++) {
            promises.push(
              $.getJSON(appSettings.dataURL[i], function(data) {
                jsonObj = jsonObj + JSON.stringify(data) + ",";
              })
          ...
          $.when.apply($, promises).done(function() {
            ...
            jsonObj = '{ "media": [' + jsonObj + "]}";
            var contentData = JSON.parse(jsonObj);
            console.log(JSON.stringify(contentData));
            element.handleJsonData(contentData);
            dataLoadedCallback();
          });
          //////////////////////////////
          ...
          this.rootFolder = this.folders[0]; 
            ...
            for (var i = 0; i < this.rootFolder.contents.length; i++) {
              for (var j = 0; j < this.folders.length; j++) {
                if (this.folders[j].mediaid === this.rootFolder.contents[i].mediaid) {
        ...
        */
        this.createFoldersFromMediaData = function (jsonData) {
            ...
            for (var i = 0; i < this.mediaData.length; i++) {
              for (var k = 0; k < this.mediaData[i].playlist.length; k++) {
                    /////////////////////
                    var mediaCats = this.mediaData[i].title;
                    // create a media reference to point to the actual media object, used in the folder list
                    var mediaReference = {type: "playlist", mediaid: this.mediaData[i].playlist[k].mediaid};
                    if (mediaCats) {
                      var catIndex = categories.indexOf(mediaCats); 
                        if (catIndex < 0) { // catIndex = -1 if categories does not have medicaCats 
                            categories.push(mediaCats); // add it
                                // add the folder object for the found unique category to the overall list
                                var folderObj = {
                                  mediaid: categories.length,
                                  title: mediaCats,
                                ...
                                jsonData.folders[0].contents.push({type: "folder", mediaid: categories.length});
        ...
        ***************************/
         ...
         */  
         this.getCategoryData = function (categoryCallback) {
           ...
           for (var i = 0; i < this.folders.length; i++ ){
            if (this.rootFolder.contents[this.currentCategory].mediaid === this.folders[i].mediaid) {
          ...
          for (i = 0; i < currContents.length; i++) {
            if (currContents[i].type === "folder") {
              for (j = 0; j < this.folders.length; j++) {
                if (this.folders[j].mediaid === currContents[i].mediaid) {
                  this.folders[j].type = "subcategory";
                  contents.push(this.folders[j]);  
                }
              }
            }
            else if (currContents[i].type === "playlist") {

              for (j = 0; j < this.mediaData.length; j++) {
                for(var k = 0; k < this.mediaData[j].playlist.length; k++) {
                  if (this.mediaData[j].playlist[k].mediaid === currContents[i].mediaid) {
                            //make sure the date is in the correct format
                            if(this.mediaData[j].playlist[k].pubdate) {
                              this.mediaData[j].playlist[k].pubdate = exports.utils.formatDate(this.mediaData[j].playlist[k].pubdate);
                            }
                            contents.push(this.mediaData[j].playlist[k]);               
                          }
                        }
                      }
                    }
         ...
         */
         this.getDataFromSearch = function (searchTerm, searchCallback) {
          this.currData = [];
          for (var i = 0; i < this.mediaData.length; i++) {
            for (var k = 0; k < this.mediaData[i].playlist.length; i++) {
              if (this.mediaData[i].playlist[k].title.toLowerCase().indexOf(searchTerm) >= 0 || this.mediaData[i].playlist[k].description.toLowerCase().indexOf(searchTerm) >= 0) {
                    //make sure the date is in the correct format
                    if(this.mediaData[i].playlist[k].pubdate) {
                      this.mediaData[i].playlist[k].pubdate = exports.utils.formatDate(this.mediaData[i].playlist[k].pubdate);
                    }
